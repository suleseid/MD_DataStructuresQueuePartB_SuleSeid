...
namespace MD_DataStructuresQueuePartB_SuleSeid
{
    ...
    public class QueueNodes<T>
    {
        ...
        public QueueNodes<T> Next { get; set; }// Reference to the next node in the stack

        // Constructor to create a new node with a value
        public QueueNodes(T value)
        {
            Value = value;
            Next = null;
        }
    ...
    {
        private QueueNodes<int> top; // Top element of the stack
        // We have to add an element on the top of the stack
        public void Push(int value)
        {
            QueueNodes<int> newNode = new QueueNodes<int>(value);
            newNode.Next = top;
            top = newNode;
        }
